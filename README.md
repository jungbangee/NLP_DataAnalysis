...
| :--- | :--- | :--- |
| **[1] AI 강의분석 리포트 시스템** | **STT 기반 강의 텍스트 AI 분석 및 강사 역량 평가 대시보드**<br>• Gemini AI를 활용한 5개 카테고리 강의 품질 자동 분석<br>• 강사별 종합평가 · 역량 레이더 차트 · 강사 간 비교 대시보드<br>• PDF · DOCX 내보내기 및 MongoDB 기반 이력 관리 | `Node.js` `MongoDB` `Gemini AI` |
| **[2] ListenCarePlease** | **AI 기반 회의록 자동 생성 및 화자 태깅 서비스**<br>• Whisper(STT)와 LangGraph Agent를 활용한 화자 분리 및 태깅<br>• 회의 유형별(6종) 맞춤형 회의록 생성 및 TODO 자동 추출<br>• RAG 기반 질의응답 및 발화 효율성 분석 대시보드 제공 | `Whisper` `LangGraph` `React` |
| **[3] QA봇** | **Llama-3.2 기반 RLHF 파이프라인 구축**<br>• `SFT` → `RM` → `PPO` → `Merge` 전 과정 구현<br>• Human Feedback을 반영한 QA 모델 정렬(Alignment)<br>• LoRA 및 4-bit Quantization을 통한 효율적 학습 | `Transformers` `TRL` `PEFT` |
| **[4] 발화자 감정 판별** | **KoBERT & FT-Transformer 기반 멀티모달 심리 분석**<br>• 대화 텍스트(Text)와 인구통계 정보(Tabular) 결합<br>• **Cross-Attention**을 활용한 Late Fusion 아키텍처 구현<br>• 발화자의 불안/우울 지수 예측 멀티태스크 학습 | `FT-Transformer` `Multimodal` |
| **[5] 예술의 전당 예매 데이터 분석** | **예술의 전당 예매 데이터 기반 매출 증대 전략 분석**<br>• 2015\~2023년 티켓 판매 빅데이터 전처리 및 EDA<br>• 고객 세분화(Member, Age) 및 시기별 매출 패턴 분석<br>• **Tableau** 대시보드를 활용한 마케팅 인사이트 도출 | `Data Visualization` |
...
1\. AI 강의분석 리포트 시스템
  ...
  * **Project Link:** [AI 강의 분석 리포트 생성기](https://github.com/jungbangee/NLP_Project/tree/main/AI%20%EA%B0%95%EC%9D%98%20%EB%B6%84%EC%84%9D%20%EB%A6%AC%ED%8F%AC%ED%8A%B8%20%EC%83%9D%EC%84%B1%EA%B8%B0)
  * **주요 목표:** STT 기반 강의 텍스트를 Gemini AI로 자동 분석하여 강사별 역량을 5개 카테고리로 평가하고, 웹 대시보드로 시각화하는 시스템 개발.
  * **핵심 기능:**
      * **5개 카테고리 자동 분석:** 언어 표현 품질 · 강의 도입 및 구조 · 개념 설명 명확성 · 예시 및 실습 연계 · 수강생 상호작용을 각각 1~5점으로 평가.
      * **강사 종합평가 AI 생성:** 전체 강의 데이터를 분석하여 역량 프로파일, 강점/약점, 개발 과제를 자동 생성.
      * **비교 대시보드:** 강사 간 카테고리 비교, 날짜별 추이, 성장률, 갭 분석 등 다각도 시각화.
      * **내보내기:** 분석 결과를 PDF · DOCX로 내보내기 (차트 이미지 포함).
      * **일괄 업로드:** 여러 강의 파일을 한 번에 분석하는 SSE 스트리밍 기반 일괄 처리.
  * **Technical Decision:**
      * **Gemini AI:** 긴 강의 텍스트(수만 토큰)를 처리할 수 있는 넓은 컨텍스트 윈도우와 한국어 성능을 고려하여 채택.
      * **SSE(Server-Sent Events):** 분석 진행 상황을 실시간으로 클라이언트에 스트리밍하기 위해 WebSocket 대신 단방향 SSE 방식 채택.
      * **MongoDB:** 강의별 카테고리 결과, 일별 요약, 강사 종합평가를 컬렉션으로 분리하여 유연한 쿼리 및 집계 처리.
  * **Troubleshooting:**
      * **Issue:** Gemini API `429 RESOURCE_EXHAUSTED` 에러로 인한 일부 카테고리 분석 실패.
      * **Solution:** 카테고리별 분석 스크립트를 독립적으로 실행하여 실패 시 해당 카테고리만 재시도 가능하도록 구조화, 분석 결과를 MongoDB에 upsert 방식으로 저장.

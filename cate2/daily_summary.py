...
def load_day(date: str, course_id: str, instructor: str) -> tuple[dict, dict]:
    """5개 카테고리 JSON을 모아 items 병합 + 메타 반환.

    returns (meta, items) — items = {"1.1": {score,reason,feedback,...}, ...}
    """
    sources = {
        "cat2": UNIFIED / f"{date}_{course_id}_{instructor}.json",
        "cat15": UNIFIED / "cate15" / f"{date}_{course_id}_report.json",
        "cat3": UNIFIED / "cate3" / f"{date}_{course_id}_{instructor}.json",
        "cat4": UNIFIED / "cate4" / f"{date}_{course_id}_{instructor}.json",
    }
    items: dict[str, dict] = {}
...
def summarize_day(date: str, course_id: str, instructor: str,
                  save: bool = True, items_override: dict | None = None) -> dict:
    """
    일자별 강의 요약을 생성합니다.

    두 가지 방식으로 호출 가능합니다.
    1. 파일에서 로드 (기본):
       summarize_day("2026-02-02", "kdt-backendj-21th", "김영아")

    2. items를 직접 주입 (웹 업로드 / 파이프라인 연동):
       summarize_day("2026-02-02", "kdt-backendj-21th", "김영아",
                     items_override={"4.1": {...}, "4.2": {...}, ...})

    Args:
        items_override: 이미 채점된 items dict를 직접 전달. None이면 파일에서 로드.
    """
    print(f"\n{'='*60}")
    ...
    if items_override is not None:
        ...
        print(f"  items 직접 주입: {len(items)}개 항목")
    else:
...
def analyze_from_upload(
    ...
    """
    웹 파이프라인에서 카테고리별 채점 결과(items)를 받아 일자 요약을 생성합니다.

    Args:
        items      : 전 카테고리 채점 결과 dict {"1.1": {score,reason,...}, ...}
        date       : 강의 날짜 "YYYY-MM-DD"
        course_id  : 강의 ID
        instructor : 강사명
        save       : True면 파일 저장, False면 dict만 반환 (기본 False)

    Returns:
        summarize_day() 결과 dict

    사용 예시 (웹 프레임워크):
        result = analyze_from_upload(
    ...
    """
    return summarize_day(date, course_id, instructor,
                         save=save, items_override=items)


if __name__ == "__main__":
    import argparse
    # ── server.js spawn 연동: python daily_summary.py <date> <course_id> <instructor> ──
    if len(sys.argv) == 4 and not any(a.startswith("--") for a in sys.argv[1:]):
        date_arg, course_arg, inst_arg = sys.argv[1], sys.argv[2], sys.argv[3]
        try:
            result = summarize_day(date_arg, course_arg, inst_arg)
    ...
    if args.test:
        ...
        summarize_day(args.date, args.course, args.instructor)
    else:
